...
nFIRE_HANDLER(naim_needpass) {
	...
	firetalk_sendpass(conn->conn, pass);
}
...
buddywin_t *cgetwin(conn_t *conn, const char *_roomname) {
	const char *roomname = _roomname;
	buddywin_t	*bwin;

	if (*roomname != ':')
		roomname = strdup(firetalk_chat_normalize(conn->conn, roomname));
...
conn_t	*naim_newconn(int proto) {
	...
	{
		conn->conn = firetalk_create_handle(proto, conn, secs_getvar_int("dofork"));
